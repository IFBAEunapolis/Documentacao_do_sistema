#1.3. Requisitos Funcionais

###1.3.1. Dos cursos

| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> O sistema deve permitrir o cadastro de novos cursos e seus atributos (coodigo e nome).</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Esta funcionalidadade tem por objetivo fazer as consulta dos cursos cadastrados no sistemas. O usuário fará a consulta usando o nome do curso como parametro de busca.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Alterar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade permitirá a exclusão de um curso cadastrado desde que não nenhuma turma esja relacionado com o curso.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade permitirá a exclusão de um curso cadastrado desde que não nenhuma turma esja relacionado com o curso.</p> |

###1.3.2. Dos Locais Físicos

| **CODIGO**     | RF004   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar Local Físico|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É necessário manter os dados dos locais físicos pois para que as aulas sejam dadas é necessário que a sala comporte os alunos matriculados na disciplina.</p> |

| **CODIGO**     | RF005  |
| :------:      | :---------- |
|  **NOME**     | Consultar Locais Físicos|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Haverão momentos em que será preciso saber quais locais estão disponíveis para dar aula.</p> |

| **CODIGO**     | RF006  |
| :------:      | :---------- |
|  **NOME**     | Apagar Locais Físicos|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">A instituição pode passar por reformas e por isso alguns locais físicos podem deixar de existir.</p> |

###1.3.2. Dos Períodos

| **CODIGO**     | RF004   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar Período|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Nesta funcionalidade será possivel cadastrar os periodos de aulas e seus atributos(codigo, ano, data de inicio, data de termino e situação). </p> |

| **CODIGO**     | RF005  |
| :------:      | :---------- |
|  **NOME**     | Consultar Período|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Nesta função o usuário será capaz de buscar quais os periodos de aula cadastrados no sistema.</p>|

| **CODIGO**     | RF006  |
| :------:      | :---------- |
|  **NOME**     | Alterar Período|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify"> Aqui será possivel alterar os dados de um periodo cadastrado (ano, data de inicio, data de termino e a situação, booleano que indica se o periodo está ativo ou não).</p> |
###1.3.3. Das Disciplinas 
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Disciplina|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> O sistema permitirá a cadastro das disciplinas e seus atributos(Codigo, nome, carga horaria, periodo, ementa e a qual ela pertence ) </p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Disciplina|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |



| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Disciplina|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |
###1.3.4. Dos Horarios de Aula
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |

###1.3.5. Dos Periodos Letivos
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |
###1.3.6. Dos Períodos Letivos por Professor
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |
###1.3.7. Dos Planos de Aula
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |

###1.3.8. Dos Planos Letivos
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |

###1.3.9. Dos Professores
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |

###1.3.10. Dos Período Letivo por professor
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |

###1.3.11. Dos Semestres

| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |

###1.3.12. Das Turmas
| **CODIGO**     | RF001   |
| :------:      | :---------- |
|  **NOME**     | Cadastrar  Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> É importante para todo sistema manter os cursos da instituição no sistema pois ele representa a primeira entidade/camada a ser cadastrada.</p> |

| **CODIGO**     | RF002  |
| :------:      | :---------- |
|  **NOME**     | Consultar de Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** | <p style="text-align:justify"> Fazer a consulta de cursos será um processo importante na associação do mesmo com alguma outra entidade.</p> |

| **CODIGO**     | RF003  |
| :------:      | :---------- |
|  **NOME**     | Apagar Curso|
|  **NIVEL**     |Essencial|
|  **DESCRIÇÃO** |  <p style="text-align:justify">Essa funcionalidade se faz necessária no caso da instituição optar por retirar uma oferta de curso.</p> |
